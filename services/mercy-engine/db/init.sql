...
-- Gates are the exception -- a PIN, a folder password, four security answers
-- are not evidence, and a participant locked out of an app is not playing.
--
-- `goto_tab` / `goto_focus` are where the console should take them when the
-- words are not enough on their own: the tab, and an element the console knows
-- how to reveal on it. Only ids the console actually owns are ever named here
-- ('victim-popover', the card that carries her date of birth); the laptop and
-- the map are sent as a tab alone, because the shell and the model have no
-- focus vocabulary of their own.
--
...
INSERT INTO context_hints (screen, detail, step, body, goto_tab, goto_focus) VALUES
