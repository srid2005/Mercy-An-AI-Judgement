...
const defState = {
  ...
  view: 1,
  showHidden: false,
  unlocked: [], // ids of password-protected folders opened this session
};
...
const fileReducer = (state = defState, action) => {
  ...
  } else if (action.type === "FILEUNLOCK") {
    tmp.unlocked = [...tmp.unlocked, action.payload];
  } else if (action.type === "FILEHIDDEN") {
    tmp.showHidden = !tmp.showHidden;
