-- city-map: vehicle tracking for a running game -- the chase, and the other
-- cars on the ANPR log (init scripts only run on a fresh volume).
--   docker compose exec -T city-map-db sh -c 'psql -U "$POSTGRES_USER" -d "$POSTGRES_DB"' < services/city-map/scripts/migrate_live_vehicles.sql
-- Idempotent: safe to run again.
BEGIN;
ALTER TABLE vehicles ADD COLUMN IF NOT EXISTS mode TEXT NOT NULL DEFAULT 'loop';
-- Nikhil's car chases; it is first picked up on Harrow Road, not at stop 1
UPDATE vehicles SET mode = 'chase', lat = 12.9862, lng = 77.5081,
  note = $story$Read by six ANPR cameras between 02:58 and 05:33 last night and still circulating; the last read was CCTV-36 on Harrow Road, minutes ago. The plate ends in the four digits of the address the threats came from.$story$
WHERE slug = 'nikhil';
-- ---------------------------------------------------------------------------
